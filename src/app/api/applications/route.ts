...
const error = (status = 400) => NextResponse.json({ message: "We couldn't submit your application. Please try again." }, { status, headers: { "Cache-Control": "no-store, max-age=0" } });
...
export async function POST(request: Request) {
 ...
 try {
  const config=getServerConfig(); const body=await request.json() as {application:unknown;uploads:Upload[]}; const application=finalSubmissionSchema.parse(body.application);
  ...
  const previous=await client.rpc("lookup_application_receipt",{p_id:id,p_fingerprint:fingerprint});if(previous.error)throw new Error();if(typeof previous.data==="string")return NextResponse.json({reference:previous.data},{headers:{"Cache-Control":"no-store"}});
