...
test("Postgres migrations deny applicant access, atomically persist records and deduplicate receipts", async () => {
  ...
  try {
    ...
    for (const migration of ["202609070001_application_intake.sql", "202609070002_document_reconciliation.sql", "202609080003_admin_portal.sql", "202609080004_admin_workflow_and_documents.sql"]) {
    ...
    const staffId = randomUUID(); const outsider = randomUUID();
    await db.query("insert into auth.users(id) values ($1),($2)", [staffId, outsider]);
    await db.query("insert into private.admin_staff(user_id) values ($1)", [staffId]);
    await assert.rejects(db.query("select public.admin_get_application($1,$2)", [outsider, id]), /Not authorized/);
    ...
    await assert.rejects(db.query("select public.admin_get_document_for_download($1,$2,'resume')", [outsider, id]), /Not authorized/);
